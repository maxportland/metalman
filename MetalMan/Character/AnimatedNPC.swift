...
//  MetalMan
//
//  Skeletal animation for NPC characters (vendors, etc.)
//
...
// MARK: - NPC Animation State

/// Represents the current animation state of an NPC
enum NPCAnimationState {
    case idle           // Standing idle (vendor-happy-idle)
    case waving         // Waving at approaching player (vendor-waving)
    case interacting    // Player has shop open (vendor-e-action)
    
    /// The animation file name (without extension) for this state
    var animationName: String {
        switch self {
        case .idle: return "vendor-happy-idle"
        case .waving: return "vendor-waving"
        case .interacting: return "vendor-e-action"
        }
    }
    ...
    var fallbackNames: [String] {
        switch self {
        case .idle: return ["vendor-waving"]
        case .waving: return ["vendor-happy-idle"]
        case .interacting: return ["vendor-happy-idle"]
        }
    }
    ...
    var isLooping: Bool {
        switch self {
        case .idle, .waving: return true
        case .interacting: return true
        }
    }
}
...
/// Manages skeletal animation for a single NPC instance
final class AnimatedNPC {
    let mesh: SkeletalMesh
    
    /// Current animation state
    private(set) var animationState: NPCAnimationState = .idle
    ...
    /// This NPC's own bone matrix buffer (separate from shared mesh)
    let boneMatrixBuffer: MTLBuffer
    
    /// Current bone matrices for this NPC
    ...
    init(device: MTLDevice, mesh: SkeletalMesh) {
        ...
        // Create this NPC's own bone matrix buffer
    ...
    func setAnimationState(_ newState: NPCAnimationState, resetTime: Bool = true) {
    ...
    func update(deltaTime: Float) {
        ...
        if let currentTransforms = getCurrentBoneTransforms() {
            ...
            // Update this NPC's bone matrices (not the shared mesh's)
            updateOwnBoneMatrices(animationTransforms: finalTransforms)
        }
    }
    
    /// Update this NPC's bone matrices based on animation transforms
    ...
    func draw(encoder: MTLRenderCommandEncoder,
        ...
        encoder.setVertexBuffer(boneMatrixBuffer, offset: 0, index: 2)  // Use this NPC's own bone matrices
    ...
    /// Get the vertex descriptor for skinned NPC meshes
